...
class MICN(nn.Module):
    ...
    """
    def __init__(self, params, conv_kernel=[12, 16]):
        """
        ...
        # embedding
        self.dec_embedding = DataEmbedding(self.enc_in, self.d_model, self.embed, self.freq,
                                           self.dropout)

        self.conv_trans = SeasonalPrediction(embedding_size=self.d_model, n_heads=self.n_heads,
                                             dropout=self.dropout,
                                             d_layers=self.d_layers, decomp_kernel=decomp_kernel,
        ...
        self.projection = nn.Linear(self.c_out * self.seq_len, self.num_class)
